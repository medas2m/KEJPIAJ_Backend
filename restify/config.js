...
const config = {
  user: 'MEDAS',
  password: 'Sadem12#',
  server: '192.168.100.26\\sqlexpress',
  database: 'kejpiaj',
  options: {
      encrypt: false,
  },
...
try {
  ...
  `;
      ...
      server.get("/energyCostBreakdownForChart", async (req, res) => {
        ...
        const agregaty = [
          "GOS_Chrzanow_AGR1_Ew",
          "GOS_Chrzanow_AGR2_Ew",
          "GOS_Chrzanow_AGR3_Ew",
        ];
        const pv = ["GOS_Chrzanow_PV_D_Ew", "GOS_Chrzanow_PV_G_Ew"];
        const zakup = ["GOS_Chrzanow_RG_CAL_Ez"];

        const allAliases = [...agregaty, ...pv, ...zakup];
        ...
        const query = `
          SELECT 
            ...
            RoundedDate,
            DailyIncrement
          FROM [KEJPIAJ].[dbo].[_AggregatedDataDailyView]
          WHERE 
            RoundedDate >= @startDate AND RoundedDate < @endDate
            AND SourceTable IN (${allAliases
              .map((_, i) => `@src${i}`)
              .join(",")})
        `;
        ...
        try {
          ...
          allAliases.forEach((alias, i) => {
            request.input(`src${i}`, sql.NVarChar, alias);
          });
          ...
          rows.forEach(({ SourceTable, RoundedDate, DailyIncrement }) => {
            const day = new Date(RoundedDate).getDate();
            const year = new Date(RoundedDate).getFullYear();
            const key = `${year}-${day}`;

            if (!dailyData[key]) {
              dailyData[key] = { agregaty: 0, pv: 0, zakup: 0 };
            }

            if (agregaty.includes(SourceTable)) {
              dailyData[key].agregaty += DailyIncrement;
            } else if (pv.includes(SourceTable)) {
              dailyData[key].pv += DailyIncrement;
            } else if (zakup.includes(SourceTable)) {
              dailyData[key].zakup += DailyIncrement;
            }
          });

          const dniWMiesiacu = new Date(
          ...
          const series = {
            koszt2024Narastajaco: makeSeries(
              "styczeń 2024 - Koszt całk. narastająco",
              2024,
              2,
              "Dash",
              "#b91c1c"
            ),
            koszt2024Miesieczny: makeSeries(
              "Styczeń 2024 - Koszt całk. miesięczny",
              2024,
              2,
              null,
              "#b91c1c"
            ),
            koszt2025Narastajaco: makeSeries(
              "styczeń 2025 - Koszt całk. narastająco",
              2025,
              4,
              null,
              "#b91c1c"
            ...
            oze2024Narastajaco: makeSeries(
              "styczeń 2024 - OZE narastająco",
              2024,
              2,
              "Dash",
              "#059669"
            ),
            oze2024Miesieczny: makeSeries(
              "Styczeń 2024 - OZE miesięcznie",
              2024,
              2,
              null,
              "#059669"
            ),
            oze2025Narastajaco: makeSeries(
              "styczeń 2025 - OZE narastająco",
              2025,
              4,
              null,
              "#059669"
            ...
            zakup2024Narastajaco: makeSeries(
              "Styczeń 2024 - Koszt zakupu",
              2024,
              2,
              "Dash",
              "#d97706"
            ),
            zakup2024Miesieczny: makeSeries(
              "Styczeń 2024 - Koszt miesięczny zakupu",
              2024,
              2,
              null,
              "#d97706"
            ),
            zakup2025Miesieczny: makeSeries(
              "Styczeń 2025 - Koszt miesięczny zakupu",
              2025,
              4,
              null,
              "#d97706"
          ...
          for (let day = 1; day <= dniWMiesiacu; day++) {
            const row2024 = dailyData[`2024-${day}`] || {
              agregaty: 0,
              pv: 0,
              zakup: 0,
            };
            const row2025 = dailyData[`2025-${day}`] || {
              agregaty: 0,
              pv: 0,
              zakup: 0,
            };

            const oze2024 = row2024.agregaty + row2024.pv;
            const oze2025 = row2025.agregaty + row2025.pv;

            const koszt2024 = oze2024 + row2024.zakup;
            const koszt2025 = oze2025 + row2025.zakup;

            acc.koszt[2024] += koszt2024;
            acc.koszt[2025] += koszt2025;

            acc.oze[2024] += oze2024;
            acc.oze[2025] += oze2025;

            acc.zakup[2024] += row2024.zakup;
            acc.zakup[2025] += row2025.zakup;

            // Push rounded values
            series.koszt2024Narastajaco.data.push(+acc.koszt[2024].toFixed(2));
            series.koszt2025Narastajaco.data.push(+acc.koszt[2025].toFixed(2));
            series.oze2024Narastajaco.data.push(+acc.oze[2024].toFixed(2));
            series.oze2025Narastajaco.data.push(+acc.oze[2025].toFixed(2));
            series.zakup2024Narastajaco.data.push(+acc.zakup[2024].toFixed(2));
          }

          // Wypełnienie serii stałych (miesięcznych)
          const fill = (val) => Array(dniWMiesiacu).fill(+val.toFixed(2));
          series.koszt2024Miesieczny.data = fill(acc.koszt[2024]);
          series.oze2024Miesieczny.data = fill(acc.oze[2024]);
          series.zakup2024Miesieczny.data = fill(acc.zakup[2024]);
          series.zakup2025Miesieczny.data = fill(acc.zakup[2025]);

          // Final export
          res.send([
            series.koszt2024Narastajaco,
            series.koszt2024Miesieczny,
            series.koszt2025Narastajaco,

            series.oze2024Narastajaco,
            series.oze2024Miesieczny,
            series.oze2025Narastajaco,

            series.zakup2024Narastajaco,
            series.zakup2024Miesieczny,
            series.zakup2025Miesieczny,
          ]);
        } catch (err) {
          console.error("Błąd w /energyCostBreakdownForChart:", err);
          res.send(500, { error: "Błąd serwera przy generowaniu danych." });
        }
      });
